...
<div class="min-h-screen text-white flex items-center justify-center px-4">
    <div class="bg-[#131420] p-8 rounded-lg w-full max-w-md shadow-lg">
        <h1 class="text-2xl font-bold mb-2 text-center">Edit Pegawai</h1>
        <p class="text-gray-400 text-sm text-center mb-6">Edit data pegawai di bawah ini</p>

        <form action="{{ route('pegawai.update', $pegawai->id) }}" method="POST" class="space-y-5 form-create">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ $pegawai->email }}"
                    class="w-full px-4 py-2 rounded bg-[#1a1b2e] border border-gray-700 focus:ring focus:border-blue-500 outline-none">
                @error('email')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="division_id" name="division_id"
                    ...
                    @foreach ($division as $div)
                        @if ($div->name == $pegawai->division->name)
            ...
            <div>
                ...
                <select id="level_akses" name="level_akses_id"
                    ...
                    @foreach ($level_akses as $akses)
                        @if ($akses->name == $pegawai->levelAkses->name)
                            <option value="{{ $akses->id }}" selected>
                                {{ $akses->name }}
                            </option>
                        @else     
